...
const dependencyCoverageText: Record<string, { label: string, description: string }> = {
  runtime_scanned: { label: 'Runtime dependencies scanned', description: 'A supported runtime manifest or lockfile was found and checked for known advisories.' },
  ...
  no_supported_input: { label: 'No supported dependency input', description: 'No supported resolved runtime dependency file was present in the repository.' },
...
function DependencyHealthPanel({ dataset }: { dataset: Dataset }) {
  ...
  return <section className="section-block"><div className="section-heading"><div><p className="eyebrow">Dependencies</p><h2>Runtime dependency advisory coverage</h2></div><p>Dependency files are interpreted according to their ecosystem and their role in the repository. Documentation and test requirements are shown separately from runtime dependencies.</p></div>
...
function Methods({ dataset }: { dataset: Dataset }) {
  return <section className="methods-page"><p className="eyebrow">Methods</p><h1>How Seebot reviews bioinformatics software</h1><p className="methods-lead">Seebot measures practical software-engineering characteristics that affect whether bioinformatics software can be found, understood, maintained and used reliably. The assessment combines repository metadata, source-code analysis, controlled command-line exercises and dependency-advisory checks.</p>
    ...
    <div className="method-section"><h2>Dependencies</h2><p>Dependency assessment follows the package ecosystem rather than the implementation language alone. <a href="https://google.github.io/osv-scanner/supported-languages-and-lockfiles/">OSV-Scanner’s ecosystem-specific parsers</a> inspect supported resolved manifests and lockfiles for ecosystems including Python, Rust, Java, JavaScript and C/C++. Perl projects with a <code>cpanfile</code> or <code>cpanfile.snapshot</code> are checked with <a href="https://metacpan.org/pod/cpan-audit">CPAN Audit</a>. Inputs under documentation, test or benchmark paths are reported as development-only and are not used as the runtime vulnerability count. If no supported runtime dependency input is present, the report states that coverage was unavailable rather than reporting zero vulnerabilities.</p></div>
